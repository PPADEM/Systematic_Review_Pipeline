...
fetch_scopus_data <- function(search_topics, geo_terms, start_year, topic_operator = "OR", max_records = 5000) {
  ...
  if (op == "MULTI") {
    ...
    scopus_q <- sprintf('TITLE-ABS-KEY((%s) AND (%s)) AND PUBYEAR > %d', combined_topic_str, geo_scopus_str, start_year - 1)
    category_name <- paste(names(search_topics), collapse = " & ")
    
    cat(sprintf("   -> [Scopus] Querying multi-block topics: %s (max limit: %d records)\n", category_name, max_records))
    Sys.sleep(0.3)
    ...
    tryCatch({
      ...
      map_dfr(res$entries, function(x) {
        tibble(
          database = "Scopus", query_category = category_name,
...
fetch_openalex_data <- function(search_topics, geo_terms, start_year, topic_operator = "OR", max_pages = 5) {
  ...
  if (op == "MULTI") {
    ...
    combined_topic_str <- paste(block_strs, collapse = " AND ")
    query_str <- sprintf('(%s) AND (%s)', combined_topic_str, geo_or_str)
    category_name <- paste(names(search_topics), collapse = " & ")
    
    cat(sprintf("   -> [OpenAlex] Querying multi-block topics: %s (max pages: %d, 200 items/page)\n", category_name, max_pages))
    Sys.sleep(0.3)
    ...
    block_res %>%
      mutate(
        database = "OpenAlex", query_category = category_name,
...
deduplicate_records <- function(df, target_journals = TARGET_JOURNALS) {
  ...
  clean_df <- clean_df %>%
    mutate(
      target_journal = ifelse(clean_journal_name(journal) %in% target_j_clean, "Y", "N")
    )
...
run_scoping_review <- function(
  ...
  cat(sprintf("   -> Target Journal Matches: %d / %d records flagged as 'Y'\n", sum(clean_data$target_journal == "Y"), nrow(clean_data)))
